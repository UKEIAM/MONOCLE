...
@Converter
public class ValueSetListConverter implements AttributeConverter<List<ValueSet>, PGobject> {

  private final ObjectMapper objectMapper = new ObjectMapper();

  @Override
  public PGobject convertToDatabaseColumn(List<ValueSet> valueSetList) {
    PGobject pgObject = new PGobject();
    pgObject.setType("jsonb");
    try {
      String jsonString = objectMapper.writeValueAsString(valueSetList);
  ...
  @Override
  public List<ValueSet> convertToEntityAttribute(PGobject pgObject) {
    try {
      TypeReference<List<ValueSet>> typeReference = new TypeReference<List<ValueSet>>() {
      };
