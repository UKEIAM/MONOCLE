...
public class CodeListConverter implements AttributeConverter<List<Code>, PGobject> {

  private final ObjectMapper objectMapper = new ObjectMapper();

  @Override
  public PGobject convertToDatabaseColumn(List<Code> codeList) {
    PGobject pgObject = new PGobject();
    // pgObject.setType("jsonb");
    try {
      String jsonString = objectMapper.writeValueAsString(codeList);
  ...
  @Override
  public List<Code> convertToEntityAttribute(PGobject pgObject) {
    try {
      TypeReference<List<Code>> typeReference = new TypeReference<List<Code>>() {
      };
