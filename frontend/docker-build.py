...
def get_build_command(env_file, NPM_BUILD_COMMAND, image):
    # Read environment variables from the file
    with open(env_file, 'r') as file:
        env_vars = file.readlines()

    # Filter and format environment variables for build arguments
    build_command = ["docker", "build", "-t", image, ".", "--build-arg", f"NPM_BUILD_COMMAND={NPM_BUILD_COMMAND}"]
    for var in env_vars:
        var = var.strip()
        if var.startswith('REACT_APP_'):
            build_command.extend(["--build-arg", f"{var}"])
...
if __name__ == "__main__":
    try:
        ...
        NPM_BUILD_COMMAND = f"build:{environment}"
        print(f"Performing actions for {environment} environment...")

        if environment == "development":
            image = f"{SERVICE_NAME}:latest"
            env_file = copy_env_file(environment)
            final_command = f"docker-compose up"
            build_image(get_build_command(env_file, NPM_BUILD_COMMAND, image), image)
            print("To run your image, you can use the following command: \n")
        else:
            tag_and_push(version)
            env_file = copy_env_file(environment)
            if environment in ["staging", "production"]:
                image_type = "gwis" if environment == "staging" else "kis"
                image = f"{DOCKER_REGISTRY}{SERVICE_NAME}:{version}-{image_type}"
                build_image(get_build_command(env_file, NPM_BUILD_COMMAND, image), image)
                push_image(image)
